...
class Segments_model extends CO_Core_Model
{
    ...
    {
        $query = "select group_concat(DISTINCT language) as languages from segments where language_parent='$id' OR id='$id'";
        if ($active == TRUE) {
            $query .= " AND active='1'";
        }
    ...
    public function check_segments_exists($title, $lang = '')
    {
        $fetch_row = TRUE;
