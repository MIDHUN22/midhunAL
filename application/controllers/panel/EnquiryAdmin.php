...
class EnquiryAdmin extends CO_Panel_Controller
{
    ...
    public function __construct()
    {
        ...
        $controller_config['disable_enquiry_img'] = FALSE;
        $controller_config['disable_brand_link'] = TRUE;
        $controller_config['disable_enquiry_description'] = FALSE;
        $controller_config['disable_enquiry_languages'] = FALSE;
        $this->data['controller_config'] = $controller_config;
    }

    public function index()
    {
        redirect('panel/enquiryadmin/all', 'refresh');
    }
    ...
    public function all()
    {
        ...
        $this->data['enquiry'] = $this->enquiry_model->get_all($filter);
    ...
    public function add($lang = 1)
    {
        ...
        if ($this->form_validation->run() === TRUE) {
            ...
            if ($no_error == TRUE) {
                $brand_id = $this->enquiry_model->add($input_data);
                if ($brand_id <= 0) {
                    $no_error = FALSE;
                }
            }
        ...
        $this->data['active_menu'] = 'enquiry';
        $this->data['site_content'] = 'add_enquiry';
        $this->load->view('panel/content', $this->data);
    }
    ...
    public function edit($id, $lang = 1)
    {
        ...
        $language_parent = '';
        if ($id > 0 && $lang == '1') {
            $parent_brand = $this->enquiry_model->get($id, '', true);
            $brand = $parent_brand;
        } else if ($id > 0 && $lang > 0) {
            $parent_brand = $this->enquiry_model->get($id, '', true);
            if ($parent_brand) {
                $language_parent = $id;
                $brand = $this->enquiry_model->get_by_parent($id, $lang, true);
            }
        } else {
            redirect('panel/enquiryadmin/all', 'refresh');
        }
        ...
        //$brand_languages = $this->enquiry_model->get_languages($id);
        ...
        if ($this->form_validation->run() === TRUE) {
            ...
            if ($no_error == TRUE) {
                if ($brand) {
                    ...
                    // print_r($input_data);
                    // exit;
                    $this->enquiry_model->add($input_data, $id);
                } else {
                    $input_data['created_at'] = time();
                    $input_data['created_by'] = $_SESSION['user_id'];
                    $this->enquiry_model->add($input_data);
                }
            }
            if ($no_error == FALSE) {
                $this->session->set_flashdata('error', 'Something went wrong, Please try again.');
                ...
                $this->session->set_flashdata('success', 'Saved successfully.');
                redirect('panel/enquiryadmin/edit/' . $id . '/' . $lang, 'refresh');
            }
        }
        ...
        $this->data['lang'] = $lang;
        $this->data['active_menu'] = 'enquiry';
        $this->data['site_content'] = 'edit_enquiry';
        // echo "<pre>";
        // print_r($this->data);
        // exit;
        $this->load->view('panel/content', $this->data);
    ...
    //     //edit brand based on language
    //     if ($id > 0 && $lang == '1') {
    //         $brand = $this->enquiry_model->get($id);
    //     } else if ($id > 0 && $lang > 0) {
    //         $brand = $this->enquiry_model->get_by_parent($id, $lang);
    //     } else {
    ...
    //     }
    //     $input_data['image'] = '';
    //     $this->enquiry_model->add($input_data, $brand->id);
    //     $this->session->set_flashdata('success', 'Image deleted successfully.');
    //     redirect('panel/enquiry/edit/' . $id . '/' . $lang, 'refresh');
    // }
